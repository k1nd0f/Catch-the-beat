...
class Score extends Numbers {
    ...
    Score() {
        ...
        k = 0;
        width = 25 * Res.RESOLUTION_WIDTH_SCALE;
        height = width * 1.5f;
        y = Res.HEIGHT - height;
    }
    ...
    void draw(SpriteBatch batch, float spacing, float scale, float top) {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            batch.draw(numbers.get(n), Res.WIDTH - width * scale * spacing * i - width * scale, y - top - (height * scale - height), width * scale, height * scale);
        }
    }
